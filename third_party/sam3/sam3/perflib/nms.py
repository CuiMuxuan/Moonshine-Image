    ...
    if ious.is_cuda:
        if GENERIC_NMS_AVAILABLE:
            try:
                ...
                logging.warning("torch_generic_nms failed; selecting a compatible SAM3 NMS backend: %s", error)
        if triton_enabled():
            try:
                ...
                logging.warning("Triton NMS failed; using the portable SAM3 NMS backend: %s", error)
