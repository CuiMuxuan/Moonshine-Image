...
def _edt_triton_native(data: torch.Tensor):
    """
...
def _edt_compatibility_fallback(data: torch.Tensor) -> torch.Tensor:
    from scipy.ndimage import distance_transform_edt

    data_cpu = data.detach().to(device="cpu", dtype=torch.bool).numpy()
    output = np.empty(data_cpu.shape, dtype=np.float32)
    all_foreground_distance = np.float32(np.sqrt(1e18))
    for index, mask in enumerate(data_cpu):
        if mask.all():
            output[index].fill(all_foreground_distance)
        else:
            output[index] = distance_transform_edt(mask).astype(np.float32, copy=False)
    return torch.from_numpy(output).to(device=data.device, dtype=torch.float32)


def edt_triton(data: torch.Tensor):
    """Compute EDT using Triton when possible, otherwise the portable SciPy path."""

    assert data.dim() == 3
    if not data.is_cuda or not triton_enabled():
        return _edt_compatibility_fallback(data)
    try:
        return _edt_triton_native(data)
