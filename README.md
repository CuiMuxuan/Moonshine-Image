...
| OCR 自动生成蒙版 | 📝 | 尚未实现 |
| SAM 智能选区 | 🚧 | 已接入 SAM1/SAM2/SAM2.1 单图点选/框选、SAM2 视频传播后端接口，以及 SAM3/SAM3.1 文本提示词智能选区首版 |
...
> SAM 当前作为蒙版编辑辅助能力接入，不作为 LaMa/SLBR 这类最终图片处理模型。当前支持 SAM1、SAM2 和 SAM2.1 单图点选/框选生成候选蒙版，生成结果会添加到当前图片蒙版中，再继续供 LaMa 等模型处理。
>
> SAM3/SAM3.1 文本智能选区已作为 CUDA 高配能力接入首版，默认文本模型为 `sam3_1_multiplex`。SAM1/SAM2/SAM2.1 点选/框选不直接支持自然语言文本选区。
>
...
> - SAM2：`models/sam2/sam2_hiera_tiny.pt`、`models/sam2/sam2_hiera_small.pt`、`models/sam2/sam2_hiera_base_plus.pt`、`models/sam2/sam2_hiera_large.pt`
...
SAM1 与 SAM2/SAM2.1 按各自上游 Apache-2.0 许可证记录。SAM3/SAM3.1 按 Meta `SAM License` 记录，本项目提供下载时仍需保留来源、版本、hash 和许可证说明；CPU 发布包不承诺 SAM3 文本智能选区可用。
